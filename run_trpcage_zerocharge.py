...
# Define a function to zero protein charges
def ZeroProteinCharges(mysystem, mypdb):
    for myforce in mysystem.getForces():
        if isinstance(myforce, mm.NonbondedForce):
            numatoms = mypdb.topology.getNumAtoms()
            numpatoms = 284
            for i in range(0,numpatoms):
                [charge, sigma, epsilon] = myforce.getParticleParameters(i)
                myforce.setParticleParameters(i, 0, sigma, epsilon)

# Zero the protein charges
ZeroProteinCharges(system, pdb)
...
# A: Christian Bale
simulation.reporters.append(app.DCDReporter('nocharge_trajectory.dcd', 100))
